...
# Fetching creds from Parameter Store
def ssm_parameter():
    ssm = boto3.client('ssm')
    parameter_email = ssm.get_parameter(Name='<SSM_PARAM_EMAIL>', WithDecryption=True)
    parameter_api_key = ssm.get_parameter(Name='<SSM_PARAM_API_KEY>', WithDecryption=True)
    parameter_j_username = ssm.get_parameter(Name='<SSM_PARAM_USER>', WithDecryption=True)
    parameter_j_password = ssm.get_parameter(Name='<SSM_PARAM_PASS>', WithDecryption=True)    
